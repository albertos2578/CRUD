...
public class PedidosDAOMYSQL implements PedidosDAO {
      ...
      private static final String Ventas_QUERY= "SELECT sum(precio) FROM carta_productos INNER JOIN pedidos ON pedidos.producto = carta_productos.nombre where EXTRACT(MONTH FROM fecha) = EXTRACT(MONTH FROM NOW());";
